  ...
  return (
    <div className={compact ? 'grid gap-2' : 'grid gap-4 md:grid-cols-5'}>
      {steps.map((step, index) => (
        ...
        >
          <label className={compact ? 'flex min-w-0 flex-1 items-start gap-3' : 'flex flex-1 flex-col'}>
            <input
              type="checkbox"
          ...
          <Link
            href={step.href}
            className={compact ? 'ml-8 mt-2 inline-flex text-xs font-semibold text-brand hover:text-white' : 'mt-4 inline-flex text-sm font-semibold text-brand hover:text-white'}
            onClick={() => {
